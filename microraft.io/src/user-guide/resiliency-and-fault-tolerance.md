...
group can tolerate failure of 1 Raft node and still remain available. 
Similarly, a 5-member Raft group can tolerate failure of 2 Raft nodes and
remain available.
...
Raft group below. In MicroRaft, a leader does not replicate log entries one by 
one. Instead, it keeps a buffer for incoming requests, and replicates their log
entries to the followers in batches, in order to improve the throughput. Once 
this buffer is filled up, the leader stops accepting new requests. In this code
sample, we allow the uncommitted log entry buffer to keep at most 10 requests.
We also slow down our followers by making them sleep for 3 seconds. Then, we
start sending requests to the leader. After some time, our requests fail with 
...
continues to accept and commit new requests. If we have a persistence 
implementation (i.e, `RaftStore`), we can recover failed Raft nodes. 
On the other hand, if we don't have persistence or cannot recover the persisted
Raft data, we can remove the failed Raft nodes from the Raft group. Please note
that when we remove a Raft node from a Raft group, the majority value is 
re-calculated based on the new size of the Raft group. In order to replace 
a non-recoverable Raft node without hurting the overall availability of 
the Raft group, we should remove the crashed Raft node first and then add 
a fresh-new one.
...
You can also see it in the [MicroRaft Github repository](https://github.com/metanet/MicroRaft/blob/master/microraft/src/test/java/io/microraft/examples/RestoreCrashedRaftNodeTest.java).
...
nodes are starting with `InMemoryRaftStore`, so after we terminate them we will
be able to get their persisted state. Once we start our Raft group, we commit 
a value via the leader, observe that value with a local query on a follower, 
and crash that follower. Then, we read the persisted state of the crashed 
follower via our `InMemoryRaftStore` object and restore it back with 
...
the operations that have side-effects because the Raft log replay process
triggers those side-effects again. Please refer to the 
...
the Raft group cannot be recovered. MicroRaft does not support any unsafe
recovery policy for now. 
...
configuration. Please refer to the [Configuration section](/user-guide/configuration/) 
to learn more about the leader election and leader heartbeat timeout
...
the majority, so the new leader will commit it. Since we replicate it for 
the second time with the new leader, we cause a duplicate commit. When we 
query the new leader, we see that there are 2 values added to the state 
machine.
...
You can also see it in the [MicroRaft Github repository](https://github.com/metanet/MicroRaft/blob/master/microraft/src/test/java/io/microraft/examples/RaftLeaderFailureTest.java).
...
the followers. Since the leader cannot talk to the majority anymore, after
the _leader heartbeat timeout_ duration elapses, our leader demotes to 
the follower role, and the followers on the other side elect a new leader
