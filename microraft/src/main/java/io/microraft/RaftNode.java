...
public interface RaftNode {
     ...
     * and can be different from the currently effective applied member list, if
     * this Raft node is part of the minority and there is an ongoing (appended
     * but not-yet-committed) membership change in the majority of the Raft
     * group. Similarly, it can be different from the current committed member
     * list of the Raft group, also if a new membership change is committed by
     * the majority Raft nodes but not learnt by this Raft node yet.
     *
     ...
     * <p>
     * Please note that the returned member list is read from the local state
     * and can be different from the committed member list, if there is an
     * ongoing (appended but not-yet committed) membership change in the Raft
     * group.
     *
     ...
     * <p>
     * The returned future is completed with {@link IllegalStateException} if
     * this Raft node has already started.
     *
     ...
     * {@link RaftNodeStatus#TERMINATED} and makes the Raft node stops executing
    ...
    /**
     * Handles the given Raft message which can be either a Raft RPC request or
     * a response.
     * <p>
     ...
     * The given operation is executed once it is committed in the Raft group,
     * and the returned future object is notified with its execution result.
     * <p>
     ...
     *
     * @param operation the operation to be replicated on the Raft group
     ...
     * local commit index is greater than or equal to the required commit index.
     * If the local commit index is smaller than the required commit index, then
     * the returned future is notified with {@link LaggingCommitIndexException}
     * so that the caller could retry its query on the same Raft node after some
     * time or forward it to another Raft node. This mechanism enables callers
     * to execute queries on Raft nodes without hitting the log replication
     * quorum and preserve monotonicity of query results. Please see the
     * <i>Section: 6.4 Processing read-only queries more efficiently</i> of the
     ...
     * @param queryPolicy    the query policy to decide how to execute the given
     *                       query
     * @param minCommitIndex the minimum commit index that this Raft node has to
     *                       have in order to execute the given query.
     * @param <T>            type of the result of the query execution
     *
     * @return the future to be notified with the result of the query execution,
     *         or the exception if the query cannot be executed
     *
     * @see QueryPolicy
     * @see NotLeaderException
     * @see CannotReplicateException
    ...
    /**
     * Replicates and commits the given membership change to the Raft group, if
     * the given group members commit index is equal to the current group
     ...
     * <p>
     * When the membership change process is completed successfully, the
     * returned future is notified with an {@link Ordered} object that contains
     * the new member list of the Raft group and the log index at which the
     * given membership change is committed.
     * <p>
     * The majority quorum size of the Raft group can increase or decrease by 1
     * if the membership change updates the number of the voting members.
     * Relatedly, since adding a new voting member increases the majority quorum
     * size, if there are already failed Raft endpoints in the Raft group, it is
     * recommended to remove them first before adding a new voting member in
     * order to avoid availability issues.
     * <p>
     * The current leader Raft node can be removed from the Raft group member
     * list safely without requiring a leadership transfer. The leader Raft node
     * commits the membership change without its own vote, and the followers
     * trigger a new leader election round on the next periodic heartbeat tick.
     * Please note that in this scenario there is a short availability gap which
     * is equal to the leader heartbeat period duration.
     * <p>
     ...
     * this case, the quorum size of the Raft group does not change. In
     * addition, a new Raft endpoint can be added to the Raft group as a
     * {@link RaftRole#FOLLOWER}, or an existing {@link RaftRole#LEARNER} Raft
     * endpoint can be promoted to {@link RaftRole#FOLLOWER} with
     * {@link MembershipChangeMode#ADD_OR_PROMOTE_TO_FOLLOWER}. In this case,
     * the quorum size of the Raft group is re-calculated based on the new
     * number of voting members. The leader Raft node does not check if the
     * given Raft endpoint's local Raft log is sufficiently up to date before
     * this re-calculation. Hence, it is the caller's responsibility to check
     * the promoted Raft endpoint's local Raft log in order to prevent
     * availability gaps if the quorum size will increase after the membership
     * change. This check can be done by getting {@link RaftNodeReport} from the
     * leader Raft node and comparing last log indices of the leader and the
     * {@link RaftRole#LEARNER} Raft endpoint.
     * <p>
     ...
     * {@link RaftGroupMembers#MAX_LEARNER_COUNT} {@link RaftRole#LEARNER}s in
     * the Raft group member list, then the returned future is notified with
     ...
     * Please see individual exception classes for more information.
     *
     * @param endpoint                        the endpoint to add to or remove
     *                                        from the Raft group
     * @param mode                            the type of the membership change
     * @param expectedGroupMembersCommitIndex the expected members commit index
     *
     * @return the future to be notified with the new member list of the Raft
     *         group if the membership change is successful, or the exception if
    ...
    /**
     * Transfers the leadership role to the given endpoint, if this Raft node is
     * the current Raft group leader with the {@link RaftNodeStatus#ACTIVE}
     * status and the given endpoint is in the committed member list of the Raft
     * group.
     * <p>
     ...
     * the given endpoint will be the new leader in the new term. However, it is
     * very unlikely that another endpoint will become the new leader.
     * <p>
     * The returned future is notified with an {@link Ordered} object that
     ...
     * this Raft node is not leader, {@link IllegalStateException} if the Raft
     * node status is not {@link RaftNodeStatus#ACTIVE},
     * {@link IllegalArgumentException} if the given endpoint is not a voting
     * member in the committed Raft group member list, and
     * {@link TimeoutException} if the leadership transfer process has timed out
     * w.r.t {@link RaftConfig#getLeaderHeartbeatTimeoutSecs()}.
     *
     * @param endpoint the Raft endpoint to which the leadership will be
     * transferred
     *
    ...
    interface RaftNodeBuilder {
         ...
         *
         * @param groupId the group id to create the Raft node with
         *
         ...
         *
         * @param initialGroupMembers the initial group members of the Raft group which
         *                            the Raft node belongs to
         *
         * @return the builder object for fluent calls
         */
        @Nonnull
         ...
         * Raft node is being added to a running Raft group.
         *
         * @param initialGroupMembers       the initial group members of the Raft group
         *                                  which the Raft node belongs to
         * @param initialVotingGroupMembers the list of voting members in the initial
         ...
         *
         * @param config the config object to create the Raft node with
         *
         * @return the builder object for fluent calls
         *
         * @see RaftConfig
         ...
         *
         * @param executor the Raft node executor object to be used for running
         ...
         *
         * @param transport the transport object to be used for communicating with other
         *                  Raft nodes
         *
         * @return the builder object for fluent calls
         *
         ...
         *
         * @param stateMachine the state machine object which will be used for execution
         *                     of queries and committed operations
         *
         * @return the builder object for fluent calls
         *
         * @see StateMachine
         ...
         *
         * @param store the Raft state object to be used for persisting internal Raft
         ...
         *
         * @param modelFactory the factory object to be used for creating Raft model
         *                     objects
         *
         * @return the builder object for fluent calls
         *
         ...
         *
         * @param listener the Raft node report listener object to be notified about
