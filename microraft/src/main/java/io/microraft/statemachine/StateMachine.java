 ...
 * instance. Since {@link RaftNodeExecutor} ensures the thread-safe execution of the tasks submitted by a Raft node, state machine
 * implementations do not need to be thread-safe.
 * <p>
 * A {@link StateMachine} implementation can implement {@link RaftNodeLifecycleAware} to perform initialization and clean up work
 * during {@link RaftNode} startup and termination. {@link RaftNode} calls {@link RaftNodeLifecycleAware#onRaftNodeStart()} before
 * calling any other method on {@link StateMachine}, and finally calls {@link RaftNodeLifecycleAware#onRaftNodeTerminate()} on
 * termination.
 *
 * @see RaftNode
...
public interface StateMachine {
     ...
     * so that replays do not make any difference.
     *
     * @param commitIndex
     ...
     * directly disabled via {@link RaftConfig}. However, disabling this optimization can cause longer snapshot transfer durations
     * since the bandwidth of followers is not utilized.
     *
     * @param commitIndex
    ...
    /**
     * Installs the given snapshot chunks for the given commit index, which is same with the commit index on which the last
     * snapshot is taken. The snapshot chunks are in the order they are provided to the chunk consumer parameter of {@link
     * #takeSnapshot(long, Consumer)} method.
     *
     * @param commitIndex
