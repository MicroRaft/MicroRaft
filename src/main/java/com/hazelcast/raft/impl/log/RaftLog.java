...
import com.hazelcast.raft.impl.util.ArrayRingbuffer;
...
public class RaftLog {
     ...
     */
    private final ArrayRingbuffer<LogEntry> logs;
    ...
    public void appendEntries(LogEntry... newEntries) {
        ...
        if (!checkAvailableCapacity(newEntries.length)) {
            throw new IllegalStateException(
                    "Not enough capacity! Capacity: " + logs.getCapacity() + ", Size: " + logs.size() + ", New entries: "
                            + newEntries.length);
        }
    ...
    public int setSnapshot(SnapshotEntry snapshot, long truncateUpToIndex) {
        if (snapshot.index() <= snapshotIndex()) {
            throw new IllegalArgumentException(
                    "Illegal index: " + snapshot.index() + ", current snapshot index: " + snapshotIndex());
        }
